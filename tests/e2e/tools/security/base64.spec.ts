...
test.describe('Base64 Encoder/Decoder', () => {
  test.beforeEach(async () => {
    ...
    await page.getByRole('button', { name: /Base64 Encoder\/Decoder/ }).first().click();
    await page.waitForTimeout(500);
  });

  test('should load the tool', async () => {
    await expect(page.getByRole('heading', { name: 'Base64 Encoder/Decoder' }).first()).toBeVisible({ timeout: 10000 });
  });
