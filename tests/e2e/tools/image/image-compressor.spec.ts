...
test.describe('Image Compressor', () => {
  test.beforeEach(async () => {
    ...
    await page.getByRole('button', { name: /Image Compressor/ }).first().click();
    await page.waitForTimeout(500);
  });

  test('should load the tool', async () => {
    await expect(page.getByRole('heading', { name: 'Image Compressor' }).first()).toBeVisible({ timeout: 10000 });
  });
