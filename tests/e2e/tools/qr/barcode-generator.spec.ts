...
test.describe('Barcode Generator', () => {
  test.beforeEach(async () => {
    ...
    await page.getByRole('button', { name: /Barcode Generator/ }).first().click();
    await page.waitForTimeout(500);
  });

  test('should load the tool', async () => {
    await expect(page.getByRole('heading', { name: 'Barcode Generator' }).first()).toBeVisible({ timeout: 10000 });
  });
