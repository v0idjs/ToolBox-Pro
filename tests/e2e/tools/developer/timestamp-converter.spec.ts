...
test.describe('Timestamp Converter', () => {
  test.beforeEach(async () => {
    ...
    await page.getByRole('button', { name: /Timestamp Converter/ }).first().click();
    await page.waitForTimeout(500);
  });

  test('should load the tool', async () => {
    await expect(page.getByRole('heading', { name: 'Timestamp Converter' }).first()).toBeVisible({ timeout: 10000 });
  });
