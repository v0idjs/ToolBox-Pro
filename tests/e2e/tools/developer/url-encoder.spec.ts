...
test.describe('URL Encoder/Decoder', () => {
  test.beforeEach(async () => {
    ...
    await page.getByRole('button', { name: /URL Encoder\/Decoder/ }).first().click();
    await page.waitForTimeout(500);
  });

  test('should load the tool', async () => {
    await expect(page.getByRole('heading', { name: 'URL Encoder/Decoder' }).first()).toBeVisible({ timeout: 10000 });
  });
