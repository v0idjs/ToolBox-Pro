...
test.describe('Remove Duplicate Lines', () => {
  test.beforeEach(async () => {
    ...
    await page.getByRole('button', { name: /Remove Duplicate Lines/ }).first().click();
    await page.waitForTimeout(500);
  });

  test('should load the tool', async () => {
    await expect(page.getByRole('heading', { name: 'Remove Duplicate Lines' }).first()).toBeVisible({ timeout: 10000 });
  });
